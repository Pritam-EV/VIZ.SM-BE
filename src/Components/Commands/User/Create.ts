...
export class Handler {
  ...
  public async handle(command: Command): Promise<[true, null] | [false, string[]]> {
    ...
    try {
      ...
      if (invalidLoginFields && invalidLoginFields.length > 0) {
        ...
        return [false, invalidLoginFields];
      }
      ...
      if (newLogin) {
        ...
        if (newLogin.aadhar) {
          loginFindOneFilterOrQuery.push({ aadhar: newLogin.aadhar });
        }
        if (newLogin.pan) {
          loginFindOneFilterOrQuery.push({ pan: newLogin.pan });
        }
        ...
        if (existingLogin) {
          ...
          if ((existingLogin.aadhar && newLogin.aadhar && existingLogin.aadhar == newLogin.aadhar) || (existingLogin.pan && newLogin.pan && existingLogin.pan == newLogin.pan)) {
  ...
  async #buildLogin(command: Command): Promise<[InstanceType<typeof Login>, null] | [null, string[]]> {
    try {
      ...
      if (command.aadhar) {
        newLogin.aadhar = command.aadhar;
      }
      if (command.pan) {
        newLogin.pan = command.pan;
      }
