...
export default class RazorpayServiceHelper {
     ...
     */
    public static async validateSignature(orderId: string, paymentId: string, signature: string): Promise<boolean> {
     ...
     * @returns True if valid, otherwise false.
     */
    public static async isValidAmount(amountValue: number): Promise<boolean> {
        return isFinite(amountValue) && amountValue > RazorpayServiceHelper.#minAllowedPayment;
    }
     ...
     * @returns ReceiptId
     */
    public static async generateReceiptId(userId: string): Promise<string> {
